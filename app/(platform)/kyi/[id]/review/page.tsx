...
// backend/convex/reviewQueue.ts). This is the client-facing, read-only
// status view — same pattern as the KYC review page.
...
async function getReviewEntry(id: string) {
  try {
    ...
    if (!client) return { entry: null as ReviewEntry | null, error: "Authentication is not configured." };

    const verification = await client.query(anyApi.verifications.get, { id });
    if (!verification) {
      return { entry: null as ReviewEntry | null, error: "This record doesn't exist, or you don't have access to it." };
    }
    ...
    console.error("[kyi review] Convex query failed", error);
    return { entry: null as ReviewEntry | null, error: "Review status is temporarily unavailable." };
  }
}
      ...
      <div className="max-w-2xl mx-auto">
        ...
        {!entry ? (
          ...
          <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-6 space-y-4">
            ...
            <p className="text-xs text-slate-400 uppercase tracking-wide">Priority: {entry.priority}</p>
